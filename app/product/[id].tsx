  ...
  useEffect(() => {
    const loadGood = async () => {
      try {
        ...
        // Get image URL if there's an imageId
        if (doc.imageId) {
        ...
        <View className="p-5">
          ...
          <View className="flex-row justify-between items-center mb-4">
            ...
            <View className="flex-row items-center bg-gray-50 px-3 py-2 rounded-full">
              ...
              />
              <Text className="font-rubik-medium">{good.rating}</Text>
            </View>
          </View>
          ...
          <TouchableOpacity 
            className={`py-3 rounded-lg mt-4 ${isInList ? 'bg-danger' : 'bg-primary-300'}`}
          ...
          {/* Back to List Button */}
          <TouchableOpacity 
            className="mt-3 py-3 rounded-lg border border-gray-200"
            onPress={() => router.back()}
          >
